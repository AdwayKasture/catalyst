defmodule Catalyst.AccountsTest do
  ...
  describe "deliver_user_update_email_instructions/3" do
    ...
    test "sends token through notification", %{user: user} do
      ...
      assert user_token =
               Repo.get_by(UserToken, [token: :crypto.hash(:sha256, token)], skip_user_id: true)
  ...
  describe "update_user_email/2" do
    ...
    test "updates the email with a valid token", %{user: user, token: token, email: email} do
      ...
      changed_user = Repo.get!(User, user.id, skip_user_id: true)
      assert changed_user.email != user.email
      assert changed_user.email == email
      assert changed_user.confirmed_at
      assert changed_user.confirmed_at != user.confirmed_at
      refute Repo.get_by(UserToken, [user_id: user.id], skip_user_id: true)
    end

    test "does not update email with invalid token", %{user: user} do
      assert Accounts.update_user_email(user, "oops") == :error
      assert Repo.get!(User, user.id, skip_user_id: true).email == user.email
      assert Repo.get_by(UserToken, [user_id: user.id], skip_user_id: true)
    end
  ...
  describe "generate_user_session_token/1" do
    ...
    test "generates a token", %{user: user} do
      ...
      assert user_token = Repo.get_by(UserToken, [token: token], skip_user_id: true)
  ...
  describe "confirm_user/1" do
    ...
    test "confirms the email with a valid token", %{user: user, token: token} do
      ...
      assert Repo.get!(User, user.id, skip_user_id: true).confirmed_at
      refute Repo.get_by(UserToken, [user_id: user.id], skip_user_id: true)
    end

    test "does not confirm with invalid token", %{user: user} do
      assert Accounts.confirm_user("oops") == :error
      refute Repo.get!(User, user.id, skip_user_id: true).confirmed_at
      assert Repo.get_by(UserToken, [user_id: user.id], skip_user_id: true)
    end
  ...
  describe "get_user_by_reset_password_token/1" do
    ...
    test "returns the user with valid token", %{user: %{id: id}, token: token} do
      ...
      assert Repo.get_by(UserToken, [user_id: id], skip_user_id: true)
    end
